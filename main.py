...
# Loud startup banner — prints before anything can crash
...
# Log the config state right away (without leaking the token)
...
# Fail clearly if required vars are missing, instead of crashing cryptically
    ...
    for item in items:
        ...
        if pub_date_raw:
            try:
                ...
                print(f">>> Could not parse pubDate '{pub_date_raw}': {e}", flush=True)
                pub_dt = None
...
def send_telegram(text):
    ...
    # Only attach a thread id for a real topic (not General / empty)
...
def post_daily_news():
    ...
    print(f">>> Now in {TZ}: {datetime.now(TZ)}; "
          f"window = last {WINDOW_HOURS}h", flush=True)
